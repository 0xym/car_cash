...
class FocusHandler {
  ...
  void make(FocusNode node, bool Function() isSet) =>
      _items.add(FocusItem(node, isSet));

  bool allOthersAreSet(FocusNode node) => _items
      .where((element) => element.focusNode != node)
      .every((element) => element.isSet());
  ...
  void focusNext() => _items
      .firstWhereOrNull((element) => !element.isSet())
      ?.focusNode
      .requestFocus();

  void afterSave(TextInputAction action) =>
      (action == TextInputAction.done) && allAreSet()
          ? saveForm()
          : focusNext();
