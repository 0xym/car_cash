...
class FuelTypeSelectionWidget extends StatelessWidget {
  ...
  FuelTypeSelectionWidget({this.fuelIndex, this.onChange, this.onDeleted, this.selectedType, this.selectedUnit});
  ...
  Widget build(BuildContext context) {
    ...
    final getValidUnit = (int value) { 
      final oldUnitType = selectedType != null ? fuelTypes.get(selectedType).unitType : null;
      final newUnitType = fuelTypes.get(value).unitType;
      return oldUnitType == newUnitType ? selectedUnit : fuelUnits.firstWhere(newUnitType);};
    final loc = Localization.of(context);
    return Column(children: [
      Row(crossAxisAlignment: CrossAxisAlignment.end, children: <Widget>[
          Expanded(child: DropdownButtonFormField<int>(
            decoration: InputDecoration(labelText: "${loc.tr('fuelType')} (${fuelIndex+1})"),
            onChanged: (value) => onChange(fuelIndex, value, getValidUnit(value)),
            value: selectedType,
            items: fuelTypes.keys.map((fuel) => DropdownMenuItem(value: fuel, child: Text(loc.ttr(fuelTypes.get(fuel).name)),)).toList(),)),
             if (onDeleted != null) IconButton(icon: Icon(Icons.remove_circle), onPressed: () => onDeleted(fuelIndex), color: Theme.of(context).errorColor,),
      ],),
